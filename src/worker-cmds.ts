...
export type WorkerOutData<T = WorkerOutCmd> =
    ...
    T extends WorkerOutCmd.CONFIG_UPDATED ? boolean :
    undefined;

export type Started = { ok: true, samplesPerFrame: number } | { ok: false };

export type WorkerOutMsg = WorkerOutCmd extends infer R ? R extends WorkerOutCmd ?
    [cmd: R, data: WorkerOutData<R>]
    : never : never;
...
export type WorkerInData<T = WorkerInCmd> =
    ...
    T extends WorkerInCmd.UPDATE_CONFIG ? RustpotterConfig :
